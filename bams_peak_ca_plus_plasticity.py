...
dend_L = 50 
num_of_stims = 1
active_synapses_vec = np.arange(0, 41, 5)#y axis 
# active_synapses_vec = np.arange(3)
# active_synapses_vec = np.arange(0, 10, 3)#y axis
# active_synapses_vec = [0]

# syn_branches = [1,3,7]#vertical. this is branch 1,3,7 starting from the bottom, and left to right. So Soma branch is #0, then left to right #1,#2, next layer L to R #3,4,5,6, etc.
# syn_branches = [3,1,4]#horizontal 
...
#             elif mean_max == 'max':
#             print((cell.effcai_GB))
            print(len(list(synapses[list(synapses)[0]][syn1_location])))
#             print(np.max((len(synapses[list(synapses)[0]][syn1_location][1]),1)))
#             print(len(list(synapses)[0][syn1_location]))
...
#             print('d')
    ...
    kws = dict(ha='center', va='center', color='g', weight='semibold', fontsize=10)
    for axi in axes:
        for ax in axi:
...
folder = 'C:/Users/mkbli/Dropbox/deep_dendrite/Calcium in Dendrites - Paper/Figures/'
# fig.savefig(folder + 'Figure 7/calcium_plasticity'+str(syn_branches)+'_theta_d_p = '+str(cell.e_synapses[0].theta_d_GB)+'_'+str(cell.e_synapses[0].theta_p_GB)+'.png') 
