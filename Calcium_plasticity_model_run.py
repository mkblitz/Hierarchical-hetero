...
def calcium_plasticity(num_of_sections, synapses, sim_time = 100, cell_model = 'bams', nmda_ratio = 2.38, excitatory = True, num_of_rec_locations = 30,
                       dend_L = 200, dt = 0.025, synapse_type = 'glu syn from Andras', traces = False, menorah = False, active_loc = (0, 0.5),
...
#         synapse_type = 'Behabadi'
        synapse_type = 'glu syn from Andras'
...
if __name__ == "__main__":
    ...
    #     menorah = False
        ...
        #[0.3,0.45,0.5,0.7]#
        syn_locations = [0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1]#[0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1]#
        ...
        for i in range(len(num_of_active_syns)):
            ...
            if plot_many_dendrite_subplot:
                count += rows
#         fig.savefig('C:/Users/mkbli/Dropbox/deep_dendrite/Calcium in Dendrites - Paper/Figures/Figure 3/fig '+str(time.time())[-3:]+'.png')
    ...
    #     plt.tight_layout()
    #     fig.savefig('C:/Users/mkbli/Dropbox/deep_dendrite/Figures/Homo-heterosynaptic plasticity/2 synapse heatmaps/fig '+str(time.time())[-3:]+'.png')
    ...
    #     trace_fig, trace_axis = plt.subplots(len(dt_vec), len(freq_vec), sharey = True)
    ...
    #     trace_fig2, trace_axis2 = plt.subplots(len(dt_vec), len(freq_vec), sharey = True)
    ...
    #         
    #         'sections with ### added to make different figures for eac set of locations, ## were commented out'
    #         trace_fig2, trace_axis2 = plt.subplots(len(active_synapses_vec), len(active_synapses_vec), sharey = True)
    #         trace_axis2_insets1, trace_axis2_insets2 = [], []
    #         trace_fig2.suptitle('Synapse 2, num_active_syns: ' + str(active_synapses_vec) + ', syn locations: ' + str((syn1_location, syn2_location)))
            
    #         rho_mat_1, rho_mat_2 = np.zeros((len(dt_vec),len(freq_vec))), np.zeros((len(dt_vec),len(freq_vec)))
            ...
            subplot_num = 0
    #         for freq_num in range(len(freq_vec)):
    ...
    #                 print(synapses)
            ...
            #     cbar_eff_cai.set_ticklabels(['depression','no change','potentiation'])
    ...
    #     plt.tight_layout()
    #     trace_axis[0,0].legend(), trace_axis2[0,0].legend()
    #     rho_fig.savefig('C:/Users/mkbli/Dropbox/deep_dendrite/Figures/Homo-heterosynaptic plasticity/active syns vs active syns/fig '+str(time.time())[-3:]+'.png')
...
#         active_synapses_vec = np.array([7])
#         active_synapses_vec = np.arange(1, 22, 7)#y axis
        start_1 = 210
        dt_vec = np.int0(np.linspace(-100,100,9))#x axis
#         dt_vec = np.array([-150,0,150])
#         dt_vec = np.int0(np.linspace(-150,150,3))
        syn_locs = [0.3,0.9]
...
#             extra_rho_matrices_1.append(extra_rho_mat_1), extra_rho_matrices_2.append(extra_rho_mat_2), extra_rho_matrices_3.append(extra_rho_mat_3),
    ...
    #     zero_max_1 = np.max([np.max(mat[-1,:]) for mat in eff_cai_matrices_1])
    #     zero_max_2 = np.max([np.max(mat[:,0]) for mat in eff_cai_matrices_2])
    #     zero_max = np.max((zero_max_1,zero_max_2))#max value for col/row with 0 active synapses
        
        gamma = 0.6
    #     print(zero_max, vmax)
    
    #     tick1,tick2,tick3,tick4,tick5,tick6 = 0,zero_max,zero_max+0.05,(zero_max+0.05+1)/2,1,1
    #     tick1,tick2,tick3,tick4,tick5,tick6 = 0, /vmax, ((1/3)*(vmax-zero_max)+zero_max)/vmax, ((2/3)*(vmax-zero_max)+zero_max)/vmax, 1, vmax
    ...
    #     print(tick1,tick2,tick3,tick4,tick5,tick6)
        for experiment_num in range(num_of_experiments):
            syn1_location = syn_locs[prox_loc_options[experiment_num]]
    ...
    #             cbar = fig.colorbar(fig2, cax=cbar_ax, )
            ...
            fig.add_subplot(ax3)
            fig.add_subplot(ax4)
#         fig.savefig('C:/Users/mkbli/Dropbox/deep_dendrite/Calcium in Dendrites - Paper/Figures/Figure 6/fig '+str(time.time())[-3:]+'.png')
    ...
    #             cbar.set_ticklabels(np.round([tick1,tick6],3))
                fig.savefig('C:/Users/mkbli/Dropbox/deep_dendrite/Calcium in Dendrites - Paper/Figures/Figure 6/sup fig '+str(experiment_num)+'.png')
